...
// INPUTS MANAGEMENT
    public bool shouldUpdateInputs = false;
    ...
    void Awake()
    {
        ...
        playerControls = objectToFind.GetComponent<PlayerControls>();
    ...
    void Start()
    {
        sprintKey = playerControls.sprint;
        jumpKey = playerControls.jump;
        crouchKey = playerControls.crouch;
        interactKey = playerControls.interact;
        zoomKey = playerControls.zoom;
    }
    ...
    void Update()
    {
        ...
        if(CanMove)
        {
            HandleMovementInput();
            HandleMouseLook();
            defaultYPos = characterController.height - 1f;
    ...
    private void HandleMouseLook()
    {
        ...
        if(Mathf.Abs(moveDirection.x) == 0f || Mathf.Abs(moveDirection.x) == 0f)
            playerCamera.transform.localPosition = new Vector3(0, characterController.height - 1f, 0.171f);
    }
    ...
    {
        Debug.DrawRay(playerCamera.ViewportToWorldPoint(interactionRayPoint), Vector3.forward, Color.red);
